...
/** A single step in the cooking process (1:1 with Rust HowToCookStep). */
export interface HowToCookStep {
  step: number;
  text: string;
  time_minutes: number;
}

/** AI-generated tip for optimising an ingredient (1:1 with Rust OptimizationTip). */
...
/** FAQ entry generated by AI (1:1 with Rust LabComboFaq). */
export interface LabComboFaq {
  question: string;
  answer: string;
}
 ...
 * Returns a single published lab combo page.
 */
export async function fetchLabCombo(
  slug: string,
  locale: string,
): Promise<LabComboPage | null> {
  return apiFetchFresh<LabComboPage>(
    `/public/lab-combos/${encodeURIComponent(slug)}?locale=${encodeURIComponent(locale)}`,
    [`lab-combo-${slug}`],
  );
}
 ...
 * Returns related published lab combo pages.
 */
export async function fetchRelatedCombos(
  slug: string,
  locale: string,
  limit = 6,
): Promise<LabComboRelated[]> {
  const pages = await apiFetchFresh<LabComboRelated[]>(
 ...
 * Returns "people also cook" suggestions for cross-linking.
 */
