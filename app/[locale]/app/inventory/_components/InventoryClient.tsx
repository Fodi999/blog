...
export function InventoryClient({ locale }: { locale: string }) {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* ── 3D Workspace — Visual + Simulation в одном контейнере ─────── */}
      {(view === 'visual' || view === 'simulation') && (
        <div className={sceneFullscreen
          ? 'fixed inset-0 z-50 bg-[#03070f]'
          : 'relative h-[calc(100vh-180px)] min-h-[640px] overflow-hidden rounded-2xl border border-border/60'
        }>
          {/* Visual */}
          {view === 'visual' && (
            <InventoryVisualWorkspace
              ...
              navActions={
                <div className="flex items-center gap-2">
                  <WorkspaceModeToggle value={view} onChange={setView} available={['data', 'visual', 'simulation']} />
                  <button
                    type="button"
                    title={sceneFullscreen ? 'Свернуть' : 'Развернуть'}
                    onClick={() => setSceneFullscreen((v) => !v)}
                    className="flex h-7 w-7 items-center justify-center rounded-lg border border-white/15 bg-black/40 text-white/50 backdrop-blur-sm transition-colors hover:bg-white/15 hover:text-white"
                  >
                    {sceneFullscreen ? (
          ...
          {/* Для Simulation — floating top-bar с pill + expand */}
          {view === 'simulation' && (
            <div className="pointer-events-none absolute inset-x-0 top-0 z-20 flex h-12 items-center justify-between px-3">
              ...
              <button
                ...
                title={sceneFullscreen ? 'Свернуть' : 'Развернуть'}
                onClick={() => setSceneFullscreen((v) => !v)}
                className="pointer-events-auto flex h-7 w-7 items-center justify-center rounded-lg border border-white/15 bg-black/40 text-white/50 backdrop-blur-sm transition-colors hover:bg-white/15 hover:text-white"
              >
                {sceneFullscreen ? (
                  <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                    <path d="M8 3v3a2 2 0 0 1-2 2H3"/><path d="M21 8h-3a2 2 0 0 1-2-2V3"/><path d="M3 16h3a2 2 0 0 1 2 2v3"/><path d="M16 21v-3a2 2 0 0 1 2-2h3"/>
          ...
          {/* Refresh — bottom-right corner */}
          <button
            type="button"
            onClick={() => load(true)}
            disabled={refreshing}
            className="pointer-events-auto absolute bottom-4 right-4 z-20 flex items-center gap-1.5 rounded-lg border border-white/15 bg-black/40 px-3 py-1.5 text-xs text-white/50 backdrop-blur-sm transition-colors hover:bg-white/15 hover:text-white disabled:opacity-40"
          >
